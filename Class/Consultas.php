...
class Consultas {
    ...
    CONST CARGAR_USUARIOS="SELECT p.id_persona, p.pers_nombres,p.pers_paterno,p.pers_materno,p.pers_email,u.usua_nombre_usuario FROM tg_personas p INNER JOIN ca_usuarios u ON p.id_persona=u.id_persona WHERE p.pers_vigente=1 AND u.USUA_VIGENTE=1 AND p.id_persona=?";
}
